...
function load(game_path, game_name, cb) {
  ...
  if (!game || !game.db_init || !game.db_init.game_name || !game.init || !game.turn || !game.config) {
  ...
  Game_schema.add(schema, function(err, result) {
    ...
    Game_schema.load_to_RAM(game, game.db_init.game_name, function(err, status) {
      if (err) {
        return cb(err);
      }

      console._log(`game loaded => ${game.db_init.game_name}`);
